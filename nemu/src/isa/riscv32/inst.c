...
enum {
  TYPE_R, TYPE_I, TYPE_U, TYPE_S, TYPE_JAL, TYPE_B, TYPE_JALR,
  TYPE_N, // none
};
...
// R type
  ...
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R    , R(rd) = src1 % src2);   //特殊情况处理待完成
// I type
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , JALR , R(rd) = s ->pc + 4; s->dnpc = src1 + imm);
// S type
...
// J type
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , JAL  , R(rd) = s->pc + 4; s->dnpc = s->pc + imm);
// B type
